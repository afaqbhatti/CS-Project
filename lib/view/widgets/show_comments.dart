...
class _ShowCommentCardState extends State<ShowCommentCard> {
  ...
  Future<void> _pullComments(ctx) async {
    Provider.of<CommentViewModel>(context, listen: false).fetchAllComments({'id': '${widget.post?.id}'}, '${authToken}');
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<Comment?> comments = Provider.of<CommentViewModel>(context).getAllComments;
                ...
                child: Container(
                  ...
                  child: ListView(
                    children: [
                            ...
                            children: <Widget>[
                              Padding(
                                ...
                                child: ClipRRect(
                                  ...
                                  child: Image.network(
                                    '${Constants.profileImage(authUser)}', width: 40, height: 40,fit: BoxFit.cover,
                              ...
                              IconButton(
                                ...
                                onPressed: () async {
                                  if (_commentTxtController.text.isEmpty) {
                                    ...
                                    Map response = await _commentViewModal.storeComments(commentStoreData, '${authToken}');
                                    ...
                                    setState(() {
                                      ...
                                      Comment newCommnt=new Comment(
                                        ...
                                        text: json['text'],
                                        createdat: NpDateTime.fromJson(json['createdat']),
                                        user: User(
                                            id: authUser?.id,
                                    ...
                                    if (response['success'] == true) {
                                      ...
                                      Utils.toastMessage('Your comment has been added');
                                    } else {
                      ...
                      if (_commentViewModal.getAllCommentStatus.status == Status.IDLE) ...[
                        if (comments.length == 0) ...[
                          Center(
                            child: Container(
                              color: Colors.white,
                      ...
                      ] else if (_commentViewModal.getAllCommentStatus.status == Status.BUSY) ...[
    ...
    return Padding(padding: EdgeInsets.all(10),
      child: Row(
        children: [
          InkWell(
            onTap: ()=>{
              Navigator.push(context, MaterialPageRoute(builder: (context) => OtherProfileScreen(comment.user?.id)))
            },
            child: ClipRRect(
                borderRadius: BorderRadius.circular(50),
                child: Image.network('${Constants.profileImage(comment.user)}', width: 30, height: 30,fit: BoxFit.cover,
              ...
              child: Column(
                ...
                children: [
                  InkWell(
                    ...
                    child: Text('${comment.user?.fname} ${comment.user?.lname}',style: TextStyle(fontWeight: FontWeight.bold),),
                  ),
                  Text('${comment.text}')
                ],
              ),
          ...
          Container(
            child: Column(
              ...
              children: [
                Text('${comment.createdat?.h}:${comment.createdat?.i}${comment.createdat?.a}',style: TextStyle(fontSize: 10,color: Colors.grey),),
                Text('${comment.createdat?.m}-${comment.createdat?.d}-${comment.createdat?.Y}',style: TextStyle(fontSize: 10,color: Colors.grey))
              ],
            ),
          )
        ],
