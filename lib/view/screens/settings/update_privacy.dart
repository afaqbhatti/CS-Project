...
class _UpdatePrivacyScreenState extends State<UpdatePrivacyScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if(widget.columnKey=='about'){ _updatedPrivacy=privacyValues?.about; }
    if(widget.columnKey=='city'){ _updatedPrivacy=privacyValues?.city; }
    if(widget.columnKey=='state'){ _updatedPrivacy=privacyValues?.state; }
    if(widget.columnKey=='relationship_status'){ _updatedPrivacy=privacyValues?.relationship_status; }
    if(widget.columnKey=='joining'){ _updatedPrivacy=privacyValues?.joining; }
    if(widget.columnKey=='workplace'){ _updatedPrivacy=privacyValues?.workplace; }
    if(widget.columnKey=='high_school'){ _updatedPrivacy=privacyValues?.high_school; }
    if(widget.columnKey=='hobbies'){ _updatedPrivacy=privacyValues?.hobbies; }
    if(widget.columnKey=='email'){ _updatedPrivacy=privacyValues?.email; }
    if(widget.columnKey=='phone'){ _updatedPrivacy=privacyValues?.phone; }
    if(widget.columnKey=='gender'){ _updatedPrivacy=privacyValues?.gender; }

    if(widget.columnKey=='friends'){ _updatedPrivacy=privacyValues?.friends; }
    if(widget.columnKey=='connections'){ _updatedPrivacy=privacyValues?.connections; }
    ...
    for (var v in Constants.networkList()){
      allPrivacy.add(
          InkWell(
                ...
                if(widget.columnKey=='friends' || widget.columnKey=='connections'){
                  ...
                  setState(() {
                    ...
                    if(_updatedPrivacy!.contains(key)){
                      ...
                      if(_updatedPrivacy!.contains('global') && _updatedPrivacy!.contains('public')){
                        _updatedPrivacy!.clear();
                        _updatedPrivacy!.add(key);

                      }else{
                        if(_updatedPrivacy!.contains('public') && _updatedPrivacy!.length>1 ){
                          _updatedPrivacy!.clear();
                          _updatedPrivacy!.add('public');
                        }else if(_updatedPrivacy!.contains('global') && _updatedPrivacy!.length>1 ){
                          _updatedPrivacy!.clear();
                          _updatedPrivacy!.add('global');
                        }
                        if(
                        _updatedPrivacy!.contains('friends') &&
                            _updatedPrivacy!.contains('connections') &&
                            _updatedPrivacy!.contains('tier-1') &&
                            _updatedPrivacy!.contains('tier-2')
                        ){
                          _updatedPrivacy!.clear();
                          _updatedPrivacy!.add('public');
                        }
            ...
            child: Container(
              color: ( _updatedPrivacy?.contains(v['key']) == true )?Colors.grey.shade300:Colors.transparent,
              padding: EdgeInsets.all(10),
              child: Row(
                children: [
