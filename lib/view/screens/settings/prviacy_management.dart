...
class _PrivacyManagementScreenState extends State<PrivacyManagementScreen> {
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Container(
                          padding: EdgeInsets.all(10),
                          child: Text('1. Social Information',style: TextStyle(fontSize: 15),),
                        ),
                        privacyCard('About','about'),
                        privacyCard('City','city'),
                        privacyCard('State','state'),
                        privacyCard('Relationship Status','relationship_status'),
                        privacyCard('Joining','joining'),
                        ...
                        Container(
                          padding: EdgeInsets.all(10),
                          child: Text('2. Network',style: TextStyle(fontSize: 15),),
                        ),
  ...
  Widget privacyCard(String column,String columnKey){
    ...
    if(privacyViewModel.fetchPrivacyStatus.status==Status.BUSY){
      ...
      if(columnKey=='about'){ image=privacyImage(privacyValues?.about); }
      if(columnKey=='city'){ image=privacyImage(privacyValues?.city); }
      if(columnKey=='state'){ image=privacyImage(privacyValues?.state); }
      if(columnKey=='relationship_status'){ image=privacyImage(privacyValues?.relationship_status); }
      if(columnKey=='joining'){ image=privacyImage(privacyValues?.joining); }
      if(columnKey=='workplace'){ image=privacyImage(privacyValues?.workplace); }
      if(columnKey=='high_school'){ image=privacyImage(privacyValues?.high_school); }
      if(columnKey=='hobbies'){ image=privacyImage(privacyValues?.hobbies); }
      if(columnKey=='email'){ image=privacyImage(privacyValues?.email); }
      if(columnKey=='phone'){ image=privacyImage(privacyValues?.phone); }
      if(columnKey=='gender'){ image=privacyImage(privacyValues?.gender); }
      if(columnKey=='friends'){ image=privacyImage(privacyValues?.friends); }
      if(columnKey=='connections'){ image=privacyImage(privacyValues?.connections); }
    }
                ...
                child: InkWell(
                  ...
                  onTap: (){
                    Navigator.push(context, MaterialPageRoute(builder: (context) =>
                        UpdatePrivacyScreen(columnKey,column))).then((value) => _pullRefresh(context));
                  },
                ),
              ),
              Text(column),
  ...
  Widget privacyImage(List<String>? privacyList){
    ...
    if(privacyList?.length == 1){
      ...
      privacyWidgets.add(
          Tooltip(
            message: '${tooltip}',
            child: Image.asset('assets/images/custom-privacy.png',
              width: 20,
              height: 20,
