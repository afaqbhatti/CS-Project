...
class PostRepo {
  ...
  Future<dynamic> getPublicPostApi(data,token) async {
    dynamic response = await _apiServices.getPostAuthApiResponse(AppUrl.fetchAllTwoPosts, data,token);
    try {
      Map res = {
  ...
  Future<dynamic> getMyPostApi(data,token) async {
    dynamic response = await _apiServices.getPostAuthApiResponse(AppUrl.fetchMyPosts, data,token);
    try {
      Map res = {
        'message': response['message'],
        'data': response['data']['allposts'],
  ...
  Future<dynamic> deleteMyPostApi(data,token) async {
    try {
      dynamic response = await _apiServices.getPostAuthApiResponse(AppUrl.deleteMyPosts, data,token);
      Map res = {
        'message': response['message'],
        'data': response['data']['post'],
      };
  ...
  Future<dynamic> createPostApi(data,token) async {
    try {
      ...
      if(data['file_type']=='image'){
        ...
        var multipartFileSign = new http.MultipartFile(data['attachment'].toString().toLowerCase(), stream, length, filename: basename(path));
        ...
        response = await _apiServices.getPostAuthApiResponse(AppUrl.createPost, data,token);
      }
