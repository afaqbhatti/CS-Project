...
class Constants {
    ...
    static List networkList({bool internal=false,bool showGlobal=true,bool showAll=false}){
        List networks=[];
        if(showAll){
    ...
    static profileImage(user){
        String profile='${AppUrl.url}storage/profile/${user?.email}/50x50${user?.profile}';
                    ...
                    children: [
                        ...
                        Container(
                            ...
                            child: InkWell(
                                ...
                                //child: (user?.anyUnreadNotification != 0 ||user?.anyUnreadNotification == null)?
                                child:Icon(
                                    Icons.notifications_none_rounded,
                                    color:Colors.black,
                                )/*:
                        Badge(
                            position: BadgePosition.topEnd(top: 12,end: 0),
                            child: Icon(
                                Icons.notifications_none_rounded,
                                color: Colors.white,
                            ),
    ...
    static authNavigation(context)async{
        ...
        if (token == null) {
            ...
            Map datum = {'id': '${authId}'};
            await Provider.of<UserViewModel>(context, listen: false).getUserDetails(datum, '${token}');
            nav='home';
        }
        return nav;
    }
    static checkVerificationStatus(context,data) async {
        if(data['email']==null){
            Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context) => AccountVerificationScreen()), (Route<dynamic> route) => false);
        }
    }
    ...
    static changeProfile(token,id,file,path,api,input)async{
        ...
        var multipartFileSign = new http.MultipartFile(input.toString().toLowerCase(), stream, length, filename: basename(path));
